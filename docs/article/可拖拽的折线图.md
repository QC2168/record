...
![image-20220315160641215](https://gitee.com/QC2168/note-img/raw/master/202203151606336.png)
...
![example](https://gitee.com/QC2168/note-img/raw/master/202203151615022.gif)

### 实现

我们先在代码中实现这一步功能，创建一个div元素并将他将给echart.init。（这个不用多说了吧，不会的同学我把链接放这了[快速上手](https://echarts.apache.org/handbook/zh/get-started/)）
...
![image-20220315184958857](https://gitee.com/QC2168/note-img/raw/master/202203151849982.png)

#### 添加可拖拽的圆点

EChart提供了多种鼠标事件类型，像平时我们常用的click、mousedown、mousemove...等事件在Echart中都可以监听得到。
...
> 为什么不是click事件？
>
> 因为click事件是在mouseup时触发的，我们需要实现的是当鼠标按住时的拖拽效果。接着在mouseup时再渲染圆点。接下去看！
...
echart支持用户绘制原生图形元素组件（`option.graphic`）
...
![3](https://gitee.com/QC2168/note-img/raw/master/202203211656565.gif)
...
![2](https://gitee.com/QC2168/note-img/raw/master/202203151624394.gif)

它对应的属性是`option.grid.tooltip.trigger`属性等于`axis`时显示。

![image-20220315162852846](https://gitee.com/QC2168/note-img/raw/master/202203151628943.png)
...
![4](https://gitee.com/QC2168/note-img/raw/master/202203211709250.gif)

#### 实现`label`标签

label标签的思想和绘制圆点一样，同样是使用`graphic`的text元素。

> 这里当时作者踩了不少的坑🙃🙃

当鼠标移到圆点上面时，显示当前圆点的标记（ID）。但是在这里不需要使用存放Label的数据，只需要一个label图形。

> 作者之前踩了一个坑，将所有的label存放到数组中（和圆点一样），结果导致的多次渲染删除标签，页面卡顿的问题
...
修改`drawAllDot`方法，在圆点图像上添加鼠标悬浮进入和移出事件。对应上面的show/hidden方法。

在图形中添加`onmouseover`，当事件触发时将当前圆点的id和对应的坐标系信息传入到showLabel方法，`onmouseout`方法则对应`hiddenLabel`方法，传入对应的id隐藏对应的label元素。
...
![5](https://gitee.com/QC2168/note-img/raw/master/202203211758795.gif)

好了，现在我们已经实现效果图了，同时本文的例子我已经放到了GITHUB上 https://github.com/QC2168/echart-drag-line，有需要看完整代码的同学可以进去看。

![耐不nice表情包-表情包网站](https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQO8WRlQZ4K9j6iQkgZjmIW9ccKy5ii2dcS9A&usqp=CAU)
